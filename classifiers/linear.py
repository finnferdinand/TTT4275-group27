...
class Linear(Classifier):
    ...
    def train(self, training_first = True):
        ...
        print("Training terminated at iteration:", iteration-1, "with ||grad MSE|| =", round(np.linalg.norm(gradient),2))
    ...
    def test(self):
        ...
        x = np.hstack((self.testv, np.ones((self.num_test * self.num_classes, 1)))).transpose()
        z = self.W @ x
        g = self._sigmoid(z)
